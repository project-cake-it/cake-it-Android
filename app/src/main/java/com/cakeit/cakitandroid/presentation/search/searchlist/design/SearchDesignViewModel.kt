            ...
            onSuccess = {
                ...
                for(i in 0 .. it.data.designs.size-1 ) {
                    for(j in 0 .. it.data.designs[i].sizes.size - 1) {
                        if(it.data.designs[i].designImages.size > 0) cakeDesigns.add(CakeDesignData(it.data.designs[i].id.toLong(), it.data.designs[i].shopAddress, it.data.designs[i].sizes[j].name, it.data.designs[i].shopName, it.data.designs[i].sizes[j].price.toLong(), it.data.designs[i].designImages[0].designImageUrl))
                        else cakeDesigns.add(CakeDesignData(it.data.designs[i].id.toLong(), it.data.designs[i].shopAddress, it.data.designs[i].sizes[j].name, it.data.designs[i].shopName, it.data.designs[i].sizes[j].price.toLong(), null))
                    }
                }
